const elements= {
  ...
  taxPaymentSubmitBtn: document.getElementById('taxPaymentSubmitBtn'),
}
  ...
  async function submitForm() {
    ...
    try {
      ...
      window.electronAPI.showResult("납세증명서가 바탕화면에 저장되었습니다.", downloadResult.success);
      setTimeout(() => window.close(), 500);  // 0.5초 후 안전하게 닫기
    } catch (err) {
...
elements.taxPaymentSubmitBtn.addEventListener('click', async ()=> submitForm());